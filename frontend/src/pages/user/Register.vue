<template lang="html">
  <div class="df-register">
    <div class="main-wrapper">
      <div class="alert">
        <el-alert
          title="亲爱的朋友，注册成功。不过还需前往注册邮箱处，激活账户才能成功登录。些许邮箱在垃圾邮件才能找到，:)"
          type="success"
          v-show="alertVisible"
          v-if="isSuccess"
        ></el-alert>
        <el-alert
          title="亲爱的朋友，注册失败。您可以稍后再来光顾，拜托啦/(ㄒoㄒ)/~~"
          type="error"
          v-show="alertVisible"
          v-else
        ></el-alert>
      ...
      <div class="main">
          ...
          <el-form class="form c-form" ref="form" :rules="rules" :model="form">
            ...
            <el-form-item prop="protocol">
              <el-checkbox v-model="form.protocol">
                阅读并同意
                <el-button type="text" @click="showDialog">double's blog  </el-button>
              </el-checkbox>
            </el-form-item>
            <el-form-item>
              <el-button
        ...
        <el-dialog class="register-dialog c-dialog" title="double's blog 用户相关和版权声明" v-model="dialogVisible">
          <span>用户同意所有以下条款后才能成为double's blog(以下简称本站)的正式注册用户。</span>
          <div class="about-user">
            <h3>本站用户相关</h3>
            <span>1. 用户在本站发布相关原文章须为用户个人创作所得。</span>
            <span>2. 用户注册成功后，即拥有在本站发布诗文(文章)、水贴、提交意见的权利。</span>
            <span>3. 不得传输或发表任何违法犯罪的、辱骂性的、淫秽的、庸俗的文章。</span>
            <span>4. 不得传输或发表任何破坏宪法和法律、破坏国家统一与煽动民族仇恨、民族歧视、破坏民族团结的言论。</span>
          </div>
          <span slot="footer">
            <el-button type="success" @click='showDialog'>同意并继续</el-button>
          </span>
        </el-dialog>
    </div>
  </div>
